...
module tablesPoc {
    ...
    export class OrganizationLocation {
        ...
        UsersCount: KnockoutObservable<number>;
        DevicesCount: KnockoutObservable<number>;
        Address: KnockoutObservable<string>;
        CityName: KnockoutObservable<string>;
        StateName: KnockoutObservable<string>;
        HasADeviceThatCanReceiveReverseAlerts: KnockoutObservable<boolean>;
        LastTestModeAlert: KnockoutObservable<string>;
    ...
    export class ViewModel {
        ...
        locations: KnockoutObservableArray<OrganizationLocation>;
        ...
        cfg = _.object(_.map(this.cfgList, v => [v["key"], v["value"]]));
        ...
        constructor() {
            this.locations = ko.observableArray(
                _.map(rawData.Locations, item => new OrganizationLocation(item)));
        }
        ...
        exampleData = {
            ...
            _backup: {},
            _setBackup: (columnKey) => {
                if (this.exampleData._backup[columnKey] == null) {
                    this.exampleData._backup[columnKey] = this.locations()[0][columnKey]();
                }
            },
            reset: (column) => {
                this.exampleData._setBackup(column.key);
                this.locations()[0][column.key](this.exampleData._backup[column.key]);
            },
            setL: (column) => {
                this.exampleData._setBackup(column.key);
                this.locations()[0][column.key](this.exampleData._L);
            },
            setXL: (column) => {
                this.exampleData._setBackup(column.key);
                this.locations()[0][column.key](this.exampleData._XL);
            }
        }
    }
}
